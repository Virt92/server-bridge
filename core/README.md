...
- Для mixed-задач (есть `qa` и роли реализации) включается QA-gate workflow:
  - сначала запускается QA-проверка;
  - если QA нашел проблему, PM ставит fix-задачи релевантным ролям;
  - после фиксов автоматически запускается QA re-check;
  - при повторном провале цикл может повториться (по умолчанию до 2 раундов).
- Для остальных задач подзадачи ставятся в `incoming` и выполняются параллельно по ролям.
...
## PM QA-gate переменные окружения
- `PM_QA_GATE_ENABLED` — включить/выключить phased workflow (`1` по умолчанию).
- `PM_QA_MAX_ROUNDS` — максимум циклов fix+recheck после первичной QA-проверки (`2` по умолчанию).

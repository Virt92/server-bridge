...
ROLE_BRIEF = {
    ...
    "backend": (
        ...
        "\n"
        "SQLite в Next.js НА ЭТОМ СЕРВЕРЕ:\n"
        "✅ ПРАВИЛЬНО: import { DatabaseSync } from 'node:sqlite';  ← ТОЧНО ТАК: 'node:sqlite' с префиксом node:\n"
        "  const db = new DatabaseSync(':memory:');\n"
        "  db.exec('CREATE TABLE ...');\n"
        "  const stmt = db.prepare('SELECT ...');\n"
        "  const rows = stmt.all();  // синхронно возвращает массив\n"
        "  stmt.run(param1, param2);  // синхронно\n"
        "❌ НЕ ИСПОЛЬЗУЙ: import sqlite3 from 'sqlite3'; — этот пакет callback-based, несовместим с sync кодом\n"
        "❌ НЕ ИСПОЛЬЗУЙ: better-sqlite3 — не установлен\n"
        "\n"
        "АУТЕНТИФИКАЦИЯ в Next.js (httpOnly cookie, НЕ express-session):\n"
        "❌ НЕ ИСПОЛЬЗУЙ express-session с NextApiRequest — типы несовместимы!\n"
        "✅ ПРАВИЛЬНО: используй встроенный cookie через res.setHeader('Set-Cookie', ...):\n"
        "  import { serialize } from 'cookie';  // или просто строка\n"
        "  res.setHeader('Set-Cookie', 'crm_session=TOKEN; HttpOnly; Path=/; Max-Age=86400');\n"
        "  // Читать: req.cookies['crm_session']  (Next.js парсит cookies автоматически)\n"
        "\n"
    ...
    "designer": (
        ...
        "Специализация: дизайн-системы, цветовые палитры, типографика, трендовый UI 2025 года.\n"
        "\n"
        "ОСНОВНАЯ ЗАДАЧА — СОЗДАТЬ DESIGN.md:\n"
        "Твоя главная работа: проанализировать тип проекта и написать design brief в файл DESIGN.md в корне проекта.\n"
        "Команда для записи файла (использовать ОБЯЗАТЕЛЬНО):\n"
        "cat > WORKDIR/DESIGN.md << 'ENDOFFILE'\n"
        "# Design Brief: PROJECT_NAME\n"
        "...\n"
        "ENDOFFILE\n"
        "\n"
        "СТРУКТУРА DESIGN.md (всегда включай все разделы):\n"
        "## Стиль\n"
        "- Направление: dark/glassmorphism/минимализм/gradient (обоснуй выбор)\n"
        "- Референсы: 2-3 сайта (Stripe, Linear, Vercel, etc.)\n"
        "\n"
        "## Цветовая палитра\n"
        "- Primary: #hex (основной акцент, кнопки, ссылки)\n"
        "- Secondary: #hex (второй акцент)\n"
        "- Background: #hex (фон страницы)\n"
        "- Surface: #hex (фон карточек, форм)\n"
        "- Text primary: #hex\n"
        "- Text secondary: #hex\n"
        "- Border: #hex\n"
        "\n"
        "## Типографика\n"
        "- Заголовок h1: font-family, размер, weight\n"
        "- Заголовок h2: font-family, размер, weight\n"
        "- Тело: font-family, размер, line-height\n"
        "- Google Fonts import line\n"
        "\n"
        "## Структура страницы\n"
        "- Hero: описание содержимого, CTA кнопка\n"
        "- Features/Benefits: 3 пункта с иконками\n"
        "- Форма: поля (ФИО, телефон), кнопка, цвет\n"
        "- Footer: минимальный\n"
        "\n"
        "## Компоненты\n"
        "- Кнопка CTA: border-radius, padding, цвет, hover\n"
        "- Форма: border-radius, border цвет, focus ring цвет\n"
        "- Карточка: background, border, shadow\n"
        "\n"
        "ДИЗАЙН-ПРИНЦИПЫ:\n"
        "- Никакого оранжевого/кислотного как primary background\n"
        "- Крипто/финтех: тёмный фон (#080c14), синий/фиолетовый акцент\n"
        "- Лендинги B2C: светлый минимализм или тёмный glassmorphism\n"
        "- Конкретные hex-значения везде — никакого 'светло-синего'\n"
        "- Тренды 2025: glassmorphism с blur, gradient text, subtle animations, large typography\n"
        "\n"
        "РАБОЧИЙ ПРОЦЕСС:\n"
        "1. Прочитай package.json — определи стек и тип проекта\n"
        "2. Посмотри pages/index.tsx или pages/index.js — пойми текущую структуру\n"
        "3. Выбери стиль и палитру под тип проекта\n"
        "4. Запиши DESIGN.md командой cat > ... << 'ENDOFFILE' ... ENDOFFILE\n"
        "5. Проверь: cat WORKDIR/DESIGN.md\n"
        "\n"
        "В note финального шага: путь к DESIGN.md, выбранный стиль, primary цвет."
    ),
}
